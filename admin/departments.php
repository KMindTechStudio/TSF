...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    $sql = "SELECT * FROM don_vi WHERE 1=1";
    $params = [];
    if ($searchKeyword !== '') {
        $sql .= " AND (ma_don_vi LIKE :kw OR ten_don_vi LIKE :kw OR email LIKE :kw)";
    ...
    foreach ($depts as $d) {
        $exportData[] = [
            ...
            'email'         => $d['email'] ?: '-',
            'trang_thai'    => ($d['trang_thai'] ?? 'active') === 'active' ? 'Đang hoạt động' : 'Ngưng hoạt động',
        ];
    }

    $columns = [
        ['key' => 'ma_don_vi', 'label' => 'Mã đơn vị', 'align' => 'center', 'width' => '130px'],
        ['key' => 'ten_don_vi', 'label' => 'Tên đơn vị công tác', 'align' => 'left', 'width' => '280px'],
        ['key' => 'so_dien_thoai', 'label' => 'Số điện thoại', 'align' => 'center', 'width' => '130px'],
        ['key' => 'email', 'label' => 'Email liên hệ', 'align' => 'left', 'width' => '220px'],
        ['key' => 'trang_thai', 'label' => 'Trạng thái', 'align' => 'center', 'width' => '130px'],
    ];

    export_to_excel('danh_sach_don_vi_' . date('Ymd_His') . '.xls', 'DANH SÁCH ĐƠN VỊ CÔNG TÁC', $columns, $exportData);
}
...
<div class="panel">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex gap-2">
            ...
            <a class="btn btn-outline-secondary" id="exportExcelBtn" href="<?= base_url('admin/departments.php?export=excel' . ($searchKeyword !== '' ? '&search=' . urlencode($searchKeyword) : '')) ?>"><i class="bi bi-file-earmark-spreadsheet me-1"></i> Xuất Excel</a>
        </div>
    </div>
