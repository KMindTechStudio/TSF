...
function export_to_excel(string $filename, string $title, array $columns, array $data): void
{
    ...
    echo 'th { background-color: #0b2d57; color: #ffffff; font-weight: bold; border: 1px solid #041935; padding: 10px 8px; text-align: center; font-size: 11pt; }';
    echo 'td { border: 1px solid #cbd5e1; padding: 7px 8px; vertical-align: middle; font-size: 10.5pt; }';
    echo 'tr:nth-child(even) { background-color: #f8fafc; }';
    echo '.school-header { font-size: 11pt; font-weight: bold; color: #1e3a8a; text-transform: uppercase; }';
    echo '.dept-header { font-size: 10pt; font-weight: bold; color: #475569; text-transform: uppercase; margin-bottom: 10px; }';
    echo '.doc-title { font-size: 16pt; font-weight: bold; color: #0b2d57; text-transform: uppercase; text-align: center; padding: 12px 0 6px 0; }';
    echo '.meta-info { font-size: 9.5pt; color: #64748b; font-style: italic; margin-bottom: 12px; }';
    ...
    echo '<div class="school-header">TRƯỜNG ĐẠI HỌC TÀI CHÍNH - NGÂN HÀNG HÀ NỘI</div>';
    echo '<div class="dept-header">KHOA CÔNG NGHỆ THÔNG TIN - HỆ THỐNG QUẢN LÝ MINH CHỨNG</div>';
    echo '<div class="doc-title">' . htmlspecialchars($title) . '</div>';
    echo '<div class="meta-info">Thời điểm xuất báo cáo: ' . htmlspecialchars($exportedAt) . ' &nbsp;|&nbsp; Người xuất: ' . htmlspecialchars($exportedBy) . '</div>';

    echo '<table>';
    echo '<thead>';
    echo '<tr>';
    echo '<th style="width: 50px;">STT</th>';
    foreach ($columns as $col) {
        $widthStyle = isset($col['width']) ? ' style="width:' . $col['width'] . ';"' : '';
        echo '<th' . $widthStyle . '>' . htmlspecialchars($col['label']) . '</th>';
    }
    echo '</tr>';
    echo '</thead>';
    echo '<tbody>';
    ...
    $stt = 1;
    if (empty($data)) {
        echo '<tr><td colspan="' . (count($columns) + 1) . '" class="text-center" style="padding: 20px; color: #94a3b8;">Không có dữ liệu phù hợp</td></tr>';
    } else {
        foreach ($data as $row) {
            echo '<tr>';
            echo '<td class="text-center text-bold">' . $stt++ . '</td>';
            foreach ($columns as $col) {
                $key = $col['key'];
                $align = $col['align'] ?? 'left';
                $classAttr = ' class="text-' . $align . '"';
                $val = $row[$key] ?? '';
                echo '<td' . $classAttr . '>' . htmlspecialchars((string) $val) . '</td>';
            }
            echo '</tr>';
        }
    }
